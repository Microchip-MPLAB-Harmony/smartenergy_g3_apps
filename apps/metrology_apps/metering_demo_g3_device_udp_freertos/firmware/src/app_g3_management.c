...
static void _ADP_DiscoveryConfirm(uint8_t status)
{
    ...
    OSAL_SEM_Post(&app_g3_managementData.semID);
}
...
static void _APP_G3_MANAGEMENT_BackoffExpired(uintptr_t context)
{
    /* Back-off time expired. Post semaphore to resume task.
     * Context holds the semaphore's address. */
    OSAL_SEM_PostISR((OSAL_SEM_HANDLE_TYPE*) context);
}
...
void APP_G3_MANAGEMENT_Initialize ( void )
{
    ...
    semResult = OSAL_SEM_Create(&app_g3_managementData.semID, OSAL_SEM_TYPE_BINARY, 0, 0);
    if ((semResult != OSAL_RESULT_SUCCESS) || (app_g3_managementData.semID == NULL))
    {
...
void APP_G3_MANAGEMENT_Tasks ( void )
{
    ...
    switch ( app_g3_managementData.state )
    {
        ...
        case APP_G3_MANAGEMENT_STATE_START_BACKOFF_DISCOVERY:
        {
            ...
            timeHandle = SYS_TIME_CallbackRegisterMS(_APP_G3_MANAGEMENT_BackoffExpired,
                    (uintptr_t) &app_g3_managementData.semID, backoffDelay, SYS_TIME_SINGLE);
        ...
        {
            /* Suspend task until back-off time expires */
            OSAL_SEM_Pend(&app_g3_managementData.semID, OSAL_WAIT_FOREVER);
            ...
            if (_APP_G3_MANAGEMENT_CheckBeaconLOADngLBPframes() == false)
            {
                ...
                /* Suspend task until network discovery finishes */
                OSAL_SEM_Pend(&app_g3_managementData.semID, OSAL_WAIT_FOREVER);
            }
            else
            {
        ...
        case APP_G3_MANAGEMENT_STATE_JOINED:
        {
             ...
             * Suspend task forever unless _LBP_ADP_NetworkLeaveIndication is
             * called */
            OSAL_SEM_Pend(&app_g3_managementData.semID, OSAL_WAIT_FOREVER);
            break;
        }
